...
import com.sm.graduation.nurse.service.NurseInfoService;
...
import com.sm.graduation.nursing.pojo.Nursing;
import com.sm.graduation.nursing.service.NursingService;
...
public class RootInfoController {
    ...
    public AjaxResult loginIn(HttpServletRequest request, HttpSession session, String username,
                              LoginPojo loginPojo, RootInfo rootInfo, NurseInfo nurseInfo, UsrInfo usrInfo){
        ...
        if (loginPojo.getCaptcha().equalsIgnoreCase(String.valueOf(request.getSession().getAttribute("captCode")))) {
            ...
            if (1 == loginPojo.getPower()){
                nurseInfo.setNurseLogin(loginPojo.getUsername());
                nurseInfo.setNursePwd(loginPojo.getPassword());
                NurseInfo nurse = nurseInfoService.sltName(nurseInfo);
                if (nurse == null){
                    return AjaxResult.error("登录名不存在");
                }
                login = nurseInfoService.loginIn(nurseInfo);
                if (login) {
                    session.setAttribute("username",nurse.getNurseName());
                    session.setAttribute("power",nurse.getPower());
    ...
    public AjaxResult nurseList(@RequestParam(defaultValue = "1" , value = "page") Integer pageNum,
                                @RequestParam(defaultValue = "10", value = "limit") Integer pageSize,
                                @RequestParam(defaultValue = ""  , value = "nurseName") String user
                                ){
        HashMap<String, Object> map = new HashMap<>();
        PageHelper.startPage(pageNum,pageSize);
        if (user == null || user.equals("")) {
            map.put("user","");
            List<NurseInfo> nurses = nurseInfoService.listAll(map);
            PageInfo<NurseInfo> page = new PageInfo<>(nurses);
            return successData(page.getTotal(), nurses);
        }
            map.put("user",user);
            List<NurseInfo> nurses = nurseInfoService.listAll(map);
            PageInfo<NurseInfo> page = new PageInfo<>(nurses);
    ...
    public AjaxResult addNurse(NurseInfo nurseInfo){
        int insert = nurseInfoService.insert(nurseInfo);
        if (1 == insert){
            return success("添加成功");
        }
        return error("添加失败");
    ...
    public AjaxResult modifyNurse(NurseInfo nurseInfo){
        int update = nurseInfoService.update(nurseInfo);
        if (1 == update){
            return success("修改成功");
        }
        return error("修改失败");
    ...
    public AjaxResult delNurse(Integer nurseId){
        nurseInfoService.delNurse(nurseId);
        return success("删除成功");
    }
    /** 批量删除管理员 */
    @RequestMapping("/batchDelNurse")
        ...
        if (null != listStr && !"".equals(listStr)){
            ...
            for (String id:ids) {
                if (null != id && !"".equals(id)) {
                    ...
                    nurseInfoService.delNurse(Integer.valueOf(id));
                }
            }
        }
        return success("删除成功");
    ...
    @RequestMapping("/nurseAltPwd")
    public AjaxResult nurseAltPwd(String pwd,String rpwd,Map map,HttpServletRequest request) {
        if(!pwd.equals(rpwd)) return error( "两次密码不一致");

        if (pwd != null && !pwd.equals("")) {
            NurseInfo nurse= (NurseInfo) request.getSession().getAttribute("nurse");
            Long id = nurse.getNurseId();
            map.put("pwd",pwd);
            map.put("id",id);
            int i = nurseInfoService.altPwd(map);
            if (i != 0) {
                return success(0, "修改成功");
            }
        }
